...
> `stringRedisTemplate.opsForValue().setIfAbsent(key, "1", 10, TimeUnit.SECONDS);`获取锁10秒后过期，如果执行时间过长，没能手动释放锁，其他线程睡眠完成后，再次查看缓存中是否有数据，如果有则返回，否则将会递归重新获取锁（只能有一个线程占有锁（类似悲观锁）），获取到锁的线程将也和第一个获取到锁的线程一样去查询数据库，然后缓存到redis，此时第一个获取到锁的线程仍然在执行，有可能出现已经缓存好了（其他线程睡醒后也可以去查到缓存了），并且释放了其他线程加的锁。
>
> 显然使用`setnx`分布式锁的情况下，如果业务执行时间长，过期时间短，可能出现多个线程查询了数据库的情况。如果锁不自动过期，当一个进程获取到锁后发生异常或者意外终止，那么其他进程将无法再获取到该锁，将导致系统出现死锁状态。通过设置适当的过期时间，即使锁的持有者发生异常，锁会在一段时间后自动释放，避免了死锁的发生。
...
> `BEGIN_TIMESTAMP = 1640995200L;`// 2022年12月31日 12:00:00 AM UTC时间秒时间戳。
>
> UTC 协调世界时，又称世界统一时间、世界标准时间、国际协调时间。由于英文（CUT）和法文（TUC）的缩写不同，作为妥协，简称UTC。协调世界时是以原子时秒长为基础，在时刻上尽量接近于世界时的一种时间计量系统。
>
> `timestamp = nowSecond - BEGIN_TIMESTAMP;`//减小时间戳（即timestamp为从BEGIN_TIMESTAMP开始递增的秒时间戳）以致于随着每天时间戳变大时能在32位空间中存放更大的整数。(2^32 - 1) / (365 * 3600 * 24) ≈ 136.097747109，32位时最多只能覆盖136年左右的时间跨度。实际上，long是有符号的64位整数，**long的高32位应该减少一位(2^31 - 1=2147483647)** / (365 * 3600 * 24) ≈ 68.1，超过68年后，时间戳将超过32位，左移32位后分布式id将会超过64位，对于long类型而言将会存储成负数的分布式id。
>
> 32位二进制数能表示的最大无符号整数是4294967295（即2^32-1二进制全为1）。而这个数以秒为单位转换成时间戳表示的时间为2106年2月7日 6:28:15 AM UTC（协调世界时）。
>
> 
>
> `stringRedisTemplate.opsForValue().increment("icr:" + keyPrefix + ":" + date);`以date中的天为key生成方便地对序列号进行统计和分析。incr命令reids高性能自增同时在高并发中能保证唯一。
>
> `timestamp << COUNT_BITS`左移32位，由时间戳占据的高32位将空间腾出来，以便使用逻辑运算符|将**redis自增长的唯一整数**拼接起来填充进去低32位。这意味着唯一的id最多占据32位，即一天之内，最多可以放4294967296（即2^32）个唯一值（reids自增时若key为空则从1开始）。所以低32位取值范围为：1-2^32-1（不受long类型影响），最终**低32位可以有2^32-2个**。
>
> 一天之内当生成的唯一的id大于2^31次方时，意味着count不仅仅是存放在低32位，低32位将全部为0，33位为1将与时间戳进行|或运算，此时id还是唯一的。若id等于2^31次方+1时，33位为1将与时间戳进行|或运算，可能会和当天生成的第一个id重复。
>
> 假设能够在一秒内生成了2^32次方+1（40多亿）个id时，这可能（50%）导致第一个id和最后一个id重复。
>
> 一天之内当生成的唯一的id等于2^64次方时，由于long是64位循环（-2^63<=long<=2^63-1）的（`Long.MAX_VALUE + 1 等于 Long.MIN_VALUE`），此时count=-2^63次方。
>
> 最终：两个long类型使用逻辑运算符或|进行拼接后，返回规律性不明显的不固定步长（高32位以秒时间戳为步长和低32位以1为步长）的自增分布式id。
...
**超时释放**：我们在加锁时增加了过期时间，这样的我们可以防止死锁，但是如果卡顿的时间超长，虽然我们采用了**lua表达式防止删锁的时候，误删别人的锁，但是毕竟没有锁住**，有安全隐患
...
注意看 commandExecutor.getConnectionManager().newTimeout() 

此方法 Method ( new TimerTask() {}, 参数2, 参数3 )

指的是：通过参数2（internalLockLeaseTime / 3），参数3 unit单位）去描述什么时候去做参数1的事情，现在的情况是：10s之后去做参数一的事情
...
> 默认10s递归续期：internalLockLeaseTime / 3 = 30 / 3 = 10，若续期成功则递归调用自己，在10s后继续续期。
>
> 一直续期到锁被释放（del key），异步的续期结果将会返回false：RFuture<Boolean> future = renewExpirationAsync(threadId);
>
> 或者线程终结无法再重复调用续期，一直等到20s后，redis数据库将会把key的对象头存活时间ttl标记成-1，此时锁将释放。
...
为了解决这个问题，redission提出来了MutiLock锁，**使用这把锁咱们就不使用主从**了，每个节点的地位都是一样的， 这把锁**加锁的逻辑需要写入到每一个主丛节点上，只有所有的服务器都写入成功，此时才是加锁成功**，假设现在某个节点挂了，那么他去获得锁的时候，只要有一个节点拿不到，都不能算是加锁成功，就保证了加锁的可靠性。
...
private class VoucherOrderHandler implements Runnable{
     ...
     public void run() {
         while (true){
             try {
                 // 1.获取队列中的订单信息
                 VoucherOrder voucherOrder = orderTasks.take();//拿取
                 // 2.创建订单
...
> ```
>
> 摘要：返回有序集合中指定分数范围内的成员，按照分数从高到低排序。
...
> 将一个或多个成员添加到地理空间索引中。如果键不存在，则创建该键。
>
> ```
...
>
> 返回由键表示的有序集合的地理空间索引中所有指定成员的位置（经度、纬度）。
>
> ```
...
@Override
    public Result queryShopByType(Integer typeId, Integer current, Double x, Double y) {
        ...
        String key = SHOP_GEO_KEY + typeId;
        GeoResults<RedisGeoCommands.GeoLocation<String>> results = stringRedisTemplate.opsForGeo() // GEOSEARCH key BYLONLAT（通过经纬度） x y BYRADIUS（按半径） 10 WITHDISTANCE（return带距离）
                .search(
                        key,
                        GeoReference.fromCoordinate(x, y),
                        new Distance(5000),
            											// includeDistance返回距离
RedisGeoCommands.GeoSearchCommandArgs.newGeoSearchArgs().includeDistance().limit(end)// 只能从零开始limit分页
                );
        // 4.解析出id
        if (results == null) {
            return Result.ok(Collections.emptyList());
...
> 		 设置或清除存储在键key对应的字符串值中，指定偏移量offset处的位。
> since: 2.2.0
> 
> GETBIT key offset
...
> - **溢出** `[WRAP|SAT|FAIL]`
>
> 在需要整数**编码**的地方，它可以通过`i`为有符号整数和`u`无符号整数加上我们整数编码的位数作为前缀来组成。因此，例如`u8`是一个 8 位的无符号整数，`i16`是一个 16 位的有符号整数。
>
> 支持的编码对于有符号整数最多为 64 位，对于无符号整数最多为 63 位。无符号整数的这种限制是由于目前 Redis 协议无法返回 64 位无符号整数作为回复。
>
> 默认情况下，如果没有另外指定，则使用**WRAP 。**
...
> 	
>   PFADD key element [element ...]（Probabilistic Filter概率性过滤器）
>   summary: HyperLogLog是一种估计集合基数的数据结构。作为一种概率性数据结构，通过高效利用空间来换取完全准确性。
>   since: 2.8.9
> 
>   PFCOUNT key [key ...]
...
![](https://i0.hdslb.com/bfs/note/1734c7d66acdcef5a6845daae4397dc7651a71c2.png)